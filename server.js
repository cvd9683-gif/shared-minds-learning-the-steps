...
export function createServer(options = {}) {
  const handleContinue = createContinueHandler(options);
  return http.createServer((req, res) => {
    ...
    if (pathname === "/api/status") {
      return send(res, 200, "application/json", JSON.stringify({
        dancerCanAnswer: Boolean(options.token),
        model: MODEL,
      }));
    }
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const maxCalls = Number(process.env.MAX_MODEL_CALLS) || 300;

  // Listen on localhost only, so nobody else on your network can spend your token.
  const log = (line) => console.log(`[continue] ${line}`);
  createServer({ token, maxCalls, apiBase: process.env.REPLICATE_API_BASE, log }).listen(port, "127.0.0.1", () => {
    console.log(`Learning the Steps is running at http://localhost:${port}`);
    const problem = tokenProblem(token);
    if (problem) console.log(`WARNING: ${problem}`);
    else console.log(token
      ? `The dancer will answer your phrases using ${MODEL} on Replicate (at most ${maxCalls} calls per run).`
      : "No REPLICATE_API_TOKEN in .env: you can teach and replay phrases; the dancer cannot answer yet, and the page says so.");
  });
}
